...
We can quickly see we would lose a lot of our data by just dropping it. Plus, we need good reasons to drop missing data (such as MCAR evidence) and so far we don't know whats missing or have any guesses as to why it is missing. 
...
# Missingness Across Relationships (Part 2)
...
# remove NAs in mean wind, mean temp, mean solar
airquality %>%
  group_by(missing) %>%
...
# Identifying Types of Missingness: Tell What You Know

## Is the variable, Ozone, MCAR? 

Our analysis shows that the Ozone data is *not* missing completely at random, both our matrix plots and our facet graph of missingness by month provide strong evidence that this is not the case. Importantly, our Ozone data is missing mostly in the month of june. 
...
## What can we conclude about missingness in Ozone?

We have now ruled out that the missingness in Ozone is MCAR, which leaves MAR and MNAR. Recall that MAR indicates we have evidence that suggests we can predict missingness from other variables in the dataset. For the purposes of this class, we just want to first and foremost be able to provide exploratory data analysis evidence that the data is MCAR, MAR, or MNAR. 

For this use case, we can conclude the variable is likely Missing At Random and that we can use other variables in the data to predict missingness. 

This means that we should not drop all missing data from the analysis nor should we impute single variables. Instead, we would want to turn to a probabilistic model for imputing the values. Which you will learn in future classes! 

## How to replace missing data

Just for practice, use a simple data frame I made. 

I'm going to already tell you I randomly sampled 7 observations from the AGE variable and made them missing. The missingness here has nothing to do with the other variables in the dataset nor does it have to do with the missing variable itself, so we are on reasonable grounds to assume the data is truly missing, or MCAR.
...
# age is our only variable with missing information
matrixplot(df, sortby = 1)

```

Though we have not done a full analysis here in detail here (where we check the relationship between each variable to see is missingness is conditional on those variables), we know the true data generating process and can proceed to demonstrate how to replace the MCAR missing data with the mean.

# Replacement

For illustration purposes, how could we replace this variable with the mean?

```{r}
# check out the replace_na function in tidyverse
df <- df %>%
  dplyr::mutate(age_replace = replace_na(age,mean(age, na.rm = TRUE)))

# summary stats won't change because we used the mean
summary(df$age)
summary(df$age_replace)
```

Because we are replacing such a low # of values (7 out of 1000), it will be hard to see differences but you can note visually that the number of cases around the mean increases.

```{r}
age_plot_1 <- ggplot(df, aes(x=age)) + 
        geom_density() + 
        labs(title="Distributions of Ages \nDropping Missing Data") + 
        theme_minimal()
        
age_plot_2 <- ggplot(df, aes(x=age_replace)) + 
        geom_density() + 
        labs(title="Distributions of Ages \nReplace Missing Data with the Mean") + 
        theme_minimal()

library(patchwork)
age_plot_1 + age_plot_2
```
