...
# answer
#life_exp_map <- full_join(life_exp, world_map, by = "region")
...
# You can also explore the setdiff function to figure out
# what kinds of renaming you might need to do 
setdiff(life_exp$reg)
```

TAKE AWAY:

- sometimes your coauthor or team mate might merge wrong! Always think through the merge/join.
- sometimes our information has different spatial labels (in this case country names) than the map we are using which can give the false appearance of MISSING DATA. You might have to manually correct this! See above.

Note, however, that the difference between two sets is order-dependent. It is the material that is in the first named set, that is not in the second named set. Thus `setdiff(A,B)` gives a different answer than `setdiff(B,A)`
...
```{r}
ggplot(life_exp_map, aes(long, lat, group = group))+
  geom_polygon(aes(fill = gdpPercap ), color = "white")+
  scale_fill_viridis_c(option = "D") + labs(title="GDP (2007)",
                                            fill="Dollars", x="Latitude", y="Longitude")
...
ggplot(arrests_map, aes(long, lat, group = group))+
  ...
  scale_fill_viridis_c(option = "C") + labs(x="Latitude", y="Longitude", title="Assaults by State") + 
  theme_bw()
...
library(gridExtra)
# arrange in one grid 
gridExtra::grid.arrange(map_1, map_2, map_3, nrow = 1) 
...
*Students do this* 
Look up the coordinates to your home or a place of interest to you.  Create a map using ggmap of your home. How specific and granular can you make the map? How broad? 
