...
beerc <- ncbreweries %>% group_by(type) %>% summarise(mean(beercount))
beerc <- data.frame(beerc)
beerc$countmean <- beerc[, c(2)]
beerc
...
```{r}
p <- ggplot(data=beerc, aes(type,countmeans))  + geom_bar(stat="identity")
p
